...
function keyparse(s) {
    ...
    const callback = (input) => {
        if (input.startsWith("<")) {
            ...
            getLast(store).delayAfter = input.length * 1000
        } else {
            input.split("").forEach((key, i) => {
                store.push({ key, delayAfter: 1000 })
            })
        }
    }

    const store = []
    const items = s.trim().split(r).filter((x) => x && x.length)
    items.forEach(callback)
    console.loggg(store)
    return store
}

async function keytrain(items, query, actions) {
    for (const item of items) {
        ...
        await sleep(item.delayAfter || 1000)
    }
}
